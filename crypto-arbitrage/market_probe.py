...
async def discover() -> tuple[list[MarketSupport], list[str]]:
    headers = {
        "User-Agent": "Police2026-Crypto-Arbitrage-Market-Probe/1.1",
...
def print_matrix(assets: list[str], quote_asset: str, supports: list[MarketSupport]) -> None:
    by_exchange = {support.exchange: support.pairs for support in supports}
    exchanges = ["binance", "kraken", "coinbase", "bitstamp"]
    print(f"{quote_asset} spot market support (live discovery)")
    print("=" * 58)
    print(f"{'PAIR':12} " + " ".join(f"{name:10}" for name in exchanges) + " VENUES")
    for asset in assets:
        ...
        print(f"{pair:12} " + " ".join(f"{cell:10}" for cell in cells) + f" {sum(flags)}")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Discover public spot markets across target exchanges")
    parser.add_argument(
        "--quote",
        default="USDT",
        help="Quote asset to check (default: USDT)",
    )
    parser.add_argument(
        "--assets",
        nargs="+",
        default=["BTC", "ETH", "SOL", "XRP", "ADA", "LINK", "DOGE", "AVAX"],
        help="Base assets to check",
    )
...
def main() -> int:
    ...
    supports, errors = asyncio.run(discover())
    print_matrix(assets, quote_asset, supports)
    for error in errors:
