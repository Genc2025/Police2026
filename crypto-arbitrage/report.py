...
def print_cross_exchange(connection: sqlite3.Connection) -> None:
    ...
    avg_net = scalar(connection, "SELECT AVG(net_profit_eur) FROM opportunities")
    ...
    if avg_net is not None:
        print(f"Average best-route P&L:EUR {avg_net:+.4f}")
    if best:
        ...
        print(f"Best estimated net:    {net_pct:+.4f}% / EUR {net_eur:+.4f}")
...
def print_triangular(connection: sqlite3.Connection) -> None:
    ...
    avg_net = scalar(connection, "SELECT AVG(net_profit_eur) FROM triangular_opportunities")
    best = connection.execute(
        """
        SELECT path, symbols, net_profit_eur, net_profit_pct, gross_profit_pct,
    ...
    if candidates:
        print(f"Qualification rate:    {(qualified / candidates) * 100.0:8.3f}%")
    if avg_net is not None:
        print(f"Average best-cycle P&L:EUR {avg_net:+.4f}")
    if best:
        path, symbols, net_eur, net_pct, gross_pct, latency, observed_at = best
